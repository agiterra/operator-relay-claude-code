#!/usr/bin/env bun
/**
 * Operator-relay MCP server for Claude Code.
 *
...
import { startRelay, stopRelay, listRelays } from "@agiterra/operator-relay-tools";
  ...
  tools: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          notify: {
            type: "string",
            description: "Agent ID to notify (your ID). Defaults to CREW_AGENT_ID.",
          },
        },
        required: ["agent"],
      },
...
mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  switch (name) {
    case "operator_relay_start": {
      ...
      const notify =
        (a.notify as string) ??
        process.env.CREW_AGENT_ID ??
        process.env.AGENT_ID;
      if (!notify) {
        return {
          content: [
            { type: "text", text: "Error: no notify agent ID — set CREW_AGENT_ID or pass notify param" },
          ],
        };
      }
